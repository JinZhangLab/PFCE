function [ b_s ] = trans_ns_pfce( Xm,Xs,b_mm)
% -------------------------------------------
...
%       b_mm (N+1)¡Á1 The linear model coefficient of master. The model can be 
...
fun = @(b_s)(([ones(size(Xm,1),1) Xm]*b_mm-[ones(size(Xs,1),1) Xs]*b_s)'*([ones(size(Xm,1),1) Xm]*b_mm-[ones(size(Xs,1),1) Xs]*b_s));%Ä¿±êº¯Êý
...
ub = [];
x0 = b_m;
b_s = fmincon(fun,x0,A,b,Aeq,beq,lb,ub,@mycon,options);
end
